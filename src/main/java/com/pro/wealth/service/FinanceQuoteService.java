...
import com.pro.wealth.model.QuoteCacheInfo;
...
public class FinanceQuoteService {
    ...
    public void populateCurrentQuote(WcInvestmentEntity wcInvestmentEntity, InvestmentInfo investmentInfo) {
        LogUtil.log("FinanceQuoteService : populateCurrentQuote : started");

        int investmentId = wcInvestmentEntity.getId();
        double amount = wcInvestmentEntity.getStockAmount();
        long diffDays = DateUtil.dateDiffInDays(new Date(), wcInvestmentEntity.getInvestmentDate());
        ...
        double currStockAmount = processAndReturnRandomQuote(investmentId, wcInvestmentEntity.getStockAmount(), diffDays, 15,  stockQuoteRefreshRateInSeconds, stockQuoteCache);
        double currMutualFundAmount = processAndReturnRandomQuote(investmentId, wcInvestmentEntity.getMutualFundAmount(), diffDays, 5,  mutualQuoteRefreshRateInSeconds, mutualQuoteCache);
        double currFdAmount = getCurrentFD(wcInvestmentEntity.getFixedDepositAmount(), diffDays);

        double currTotalAmount = currStockAmount + currMutualFundAmount + currFdAmount;
        int totalPercentageDifference = NumberUtil.percentageIncrease(wcInvestmentEntity.getInvestmentAmount(), currTotalAmount);
